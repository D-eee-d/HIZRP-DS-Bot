...
module.exports.run = async (bot, msg, args) => {
    ...
    if (user) {
        ...
        const role = msg.guild.roles.cache.find(role => role.id === config.AdminRole.muterole);

        if (member) {
            member.roles.add(role)
            .then(msg.channel.send(`Администратор <@${msg.author.id}> выдал **общую блокировку** <@${member.id}> по причине: '${args[1]}'`))
            .catch(console.error)
        } else {
